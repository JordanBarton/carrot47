...
while game == True:
    ...
    player[nplayers-2].Raise(1)#TODO:rotate the blinds
    player[nplayers-1].Raise(2)#roatate the blinds
    for n in range(0,  4):
        ...
        while(flag == 1):
                ...
                #player n+1 = players[n]
                #player n-1 always small
                #player n always big
                ...
                for i in range(0,nplayers):
                    ...
                    if player[i].cards == None:
                        player[i].fold(player[previous].bet)
                    else:
                       
                        decision = input("what will player {} do?".format(i+1))
                        if decision == "raise":
                            player[i].Raise(input("how much?"))
                            
                        if decision == "fold":
                            player[i].fold(player[previous].bet)
                        
                        if decision == "call":
                            player[i].Raise(player[previous].bet)
                            
                         
                        bets = []
                        for play in player:
                            bets.append(play.bet)
                 
                        list(filter(lambda v: v != 0, bets))
                      
                        if (len(set(bets)) <=2 and bets[nplayers-1] !=0) or (len(set(bets)) ==1 and i == nplayers-1):
                            flag = 0
                            break
    ...
    for i in range(0,nplayers):
            if player[i].cards != None:
                print(player[i].show_deck())
                value = winning_hand([river.cards[0], river.cards[1], river.cards[2] , river.cards[3],river.cards[4],player[i].cards[0],player[i].cards[1]])
            else:
                value = 0
            values.append(value)   
    ...
    for i in range(0,len(values)):
        if values[i] == maxv:
            ...
            player[i].money = player[i].money + blank.money + flop.money + turn.money + river.money
        print(player[i].money)    


    for i in range(0,nplayers):
        if player[i].money == 1000 * nplayers :
            game = False
            break
    ...
    for i in range(0,nplayers):
        if i < nplayers:#0,1,2
            player[i].cards = [playing_deck.deal(),playing_deck.deal()]
